...
FrameBuffer::FrameBuffer(int32_t bufferWidth, int32_t bufferHeight, int32_t sample)
	: bEnableMultisampling_(true)
	, width_(bufferWidth)
	, height_(bufferHeight)
{
	GL_FAILED(glGenFramebuffers(1, &framebufferID_));
	...
	for (std::size_t index = 0; index < MAX_COLOR_BUFFER_COUNT; ++index)
	{
		...
		GL_FAILED(glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, colorBufferID_[index]));
		GL_FAILED(glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, sample, GL_RGBA32F, bufferWidth, bufferHeight, GL_TRUE));
		GL_FAILED(glFramebufferTexture2D(GL_FRAMEBUFFER, attachments[index], GL_TEXTURE_2D_MULTISAMPLE, colorBufferID_[index], 0));
	}
	...
	GL_FAILED(glGenRenderbuffers(1, &depthStencilBufferID_));
	GL_FAILED(glBindRenderbuffer(GL_RENDERBUFFER, depthStencilBufferID_));
	GL_FAILED(glRenderbufferStorageMultisample(GL_RENDERBUFFER, sample, GL_DEPTH24_STENCIL8, bufferWidth, bufferHeight));
	GL_FAILED(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, depthStencilBufferID_));
